...
class CLICommand:
    ...
    @staticmethod
    def run(args):
        relax(args.krelax, args.fmax, args.geo)
...
def relax_structure(krelax, fmax, geo):
    if relax_done(fmax = fmax):
        return
    else:
        atoms = read(geo)
        formula = atoms.get_chemical_formula()
        atoms.set_calculator(Dftb(label=formula,
                                  atoms=atoms,
                                  kpts=krelax,
                                  Hamiltonian_SCC='Yes',
        ...
        opt = QuasiNewton(atoms, trajectory= formula + '.traj')
        opt.run(fmax=fmax)

        
def relax(krelax=[6, 6, 6], fmax=0.01, geo=None):
    folder = os.getcwd()
    if not geo:
    ...
    with chdir(folder + '/1-optimization'):
        with out('relax'):
            relax_structure(krelax=krelax, fmax=fmax, geo=geo[0])
